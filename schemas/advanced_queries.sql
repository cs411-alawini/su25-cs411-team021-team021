...
[Optimal]
-> Sort: avg_happy_rating DESC (actual time=3.27..3.28 rows=40 loops=1) -> Filter: (votes >= 2) (actual time=3.21..3.22 rows=40 loops=1) -> Table scan on <temporary> (actual time=3.21..3.22 rows=40 loops=1) -> Aggregate using temporary table (actual time=3.21..3.21 rows=40 loops=1) -> Nested loop inner join (cost=554 rows=333) (actual time=0.176..2.85 rows=340 loops=1) -> Filter: ((m.mood_label = 'Happy') and (m.ts >= <cache>((now() - interval 30 day))) and (m.song_id is not null)) (cost=437 rows=333) (actual time=0.158..2.28 rows=340 loops=1) -> Table scan on m (cost=437 rows=5000) (actual time=0.142..1.64 rows=5000 loops=1) -> Single-row index lookup on s using PRIMARY (song_id=m.song_id) (cost=0.25 rows=1) (actual time=0.00146..0.00148 rows=1 loops=340)
*/
...
[Optimal]
-> Sort: total_logs DESC (actual time=15.4..15.4 rows=246 loops=1) -> Filter: ((distinct_moods >= 3) and (avg_rating >= 80)) (actual time=12.5..15.3 rows=246 loops=1) -> Stream results (actual time=12.4..15.1 rows=617 loops=1) -> Group aggregate: avg(MoodLog.rating), count(0), count(distinct MoodLog.mood_label) (actual time=12.4..14.8 rows=617 loops=1) -> Sort: u.user_id (actual time=12.4..12.7 rows=5000 loops=1) -> Stream results (cost=2254 rows=5000) (actual time=0.163..10.5 rows=5000 loops=1) -> Nested loop inner join (cost=2254 rows=5000) (actual time=0.157..7.74 rows=5000 loops=1) -> Filter: (m.user_id is not null) (cost=504 rows=5000) (actual time=0.14..2.07 rows=5000 loops=1) -> Table scan on m (cost=504 rows=5000) (actual time=0.139..1.71 rows=5000 loops=1) -> Single-row index lookup on u using PRIMARY (user_id=m.user_id) (cost=0.25 rows=1) (actual time=933e-6..961e-6 rows=1 loops=5000)
*/
...
[Optimal]
-> Sort: diff DESC (actual time=2.51..2.51 rows=21 loops=1) -> Filter: (diff > 0.30) (actual time=2.32..2.49 rows=21 loops=1) -> Stream results (cost=2315 rows=5000) (actual time=2.31..2.48 rows=40 loops=1) -> Nested loop inner join (cost=2315 rows=5000) (actual time=2.3..2.44 rows=40 loops=1) -> Filter: (c.song_id is not null) (cost=1008..565 rows=5000) (actual time=2.28..2.29 rows=40 loops=1) -> Table scan on c (cost=1008..1011 rows=40) (actual time=2.28..2.29 rows=40 loops=1) -> Materialize CTE crowd (cost=1008..1008 rows=40) (actual time=2.28..2.28 rows=40 loops=1) -> Filter: ((avg((case when (MoodLog.mood_label = 'Happy') then MoodLog.rating end)) / 100) is not null) (cost=1004 rows=40) (actual time=0.198..2.24 rows=40 loops=1) -> Group aggregate: avg((case when (MoodLog.mood_label = 'Happy') then MoodLog.rating end)) (cost=1004 rows=40) (actual time=0.192..2.22 rows=40 loops=1) -> Covering index scan on MoodLog using idx_ml_songid_moodlabel_rating (cost=504 rows=5000) (actual time=0.122..1.36 rows=5000 loops=1) -> Single-row index lookup on s using PRIMARY (song_id=c.song_id) (cost=0.25 rows=1) (actual time=0.00356..0.00359 rows=1 loops=40)
*/
...
[Optimal]
-> Sort: mood_diversity DESC, total_tracks DESC (actual time=2837..2837 rows=764 loops=1) -> Stream results (cost=773526 rows=764) (actual time=3.53..2836 rows=764 loops=1) -> Group aggregate: count(distinct m.mood_label), count(0) (cost=773526 rows=764) (actual time=3.53..2834 rows=764 loops=1) -> Nested loop inner join (cost=393168 rows=3.8e+6) (actual time=0.128..1556 rows=3.82e+6 loops=1) -> Nested loop inner join (cost=3593 rows=30429) (actual time=0.0859..26.1 rows=30560 loops=1) -> Nested loop inner join (cost=345 rows=764) (actual time=0.064..7.87 rows=764 loops=1) -> Filter: (p.user_id is not null) (cost=77.4 rows=764) (actual time=0.0502..1.94 rows=764 loops=1) -> Index scan on p using PRIMARY (cost=77.4 rows=764) (actual time=0.0494..1.28 rows=764 loops=1) -> Single-row index lookup on u using PRIMARY (user_id=p.user_id) (cost=0.25 rows=1) (actual time=0.00678..0.00688 rows=1 loops=764) -> Covering index lookup on ps using PRIMARY (playlist_id=p.playlist_id) (cost=0.274 rows=39.8) (actual time=0.0132..0.0209 rows=40 loops=764) -> Covering index lookup on m using idx_moodlog_song_mood (song_id=ps.song_id) (cost=0.303 rows=125) (actual time=0.0275..0.0416 rows=125 loops=30560)
*/