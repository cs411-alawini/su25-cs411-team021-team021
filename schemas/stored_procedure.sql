...
BEGIN
    ...
    INSERT INTO PlaylistSong (playlist_id, song_id)
      ...
      FROM (SELECT song_id FROM MoodLog
            WHERE mood_label = 'Happy'
            ORDER BY RAND() LIMIT v_happy_cnt) AS t;

    INSERT INTO PlaylistSong (playlist_id, song_id)
      SELECT v_playlist_id, song_id
      FROM (SELECT song_id FROM MoodLog
            WHERE mood_label = 'Sad'
            ORDER BY RAND() LIMIT v_sad_cnt) AS t;

    INSERT INTO PlaylistSong (playlist_id, song_id)
      SELECT v_playlist_id, song_id
      FROM (SELECT song_id FROM MoodLog
            WHERE mood_label = 'Calm'
            ORDER BY RAND() LIMIT v_calm_cnt) AS t;

    INSERT INTO PlaylistSong (playlist_id, song_id)
      SELECT v_playlist_id, song_id
      FROM (SELECT song_id FROM MoodLog
            WHERE mood_label = 'Energetic'
            ORDER BY RAND() LIMIT v_energetic_cnt) AS t;

    INSERT INTO PlaylistSong (playlist_id, song_id)
      SELECT v_playlist_id, song_id
      FROM (SELECT song_id FROM MoodLog
            WHERE mood_label = 'Angry'
            ORDER BY RAND() LIMIT v_angry_cnt) AS t;
